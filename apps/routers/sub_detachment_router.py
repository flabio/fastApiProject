...
from apps.schemas.sub_detachment_schemas import SubDetachmentListSchema
...
@sub_detachment_router.get("/",dependencies=[Depends(check_admin)],response_model=List[SubDetachmentListSchema],status_code=status.HTTP_200_OK)
...
@sub_detachment_router.post("/",dependencies=[Depends(check_admin)],status_code=status.HTTP_201_CREATED)
async def create_sub_detachment(request:Request,db:session=Depends(get_db)):
    ...
    return  await SubDetachmentRepository.create_sub_detachment(validata_data,db)

@sub_detachment_router.post("/edit/{id}",dependencies=[Depends(check_admin)],status_code=status.HTTP_201_CREATED)
...
@sub_detachment_router.delete("/{id}",dependencies=[Depends(check_admin)],status_code=status.HTTP_200_OK)
